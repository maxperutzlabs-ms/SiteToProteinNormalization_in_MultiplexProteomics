...
```{r Prepare aggregation by adding new rows for multiple modifications per site. Also update PSM-ID column (MS.MS.IDs) accordingly}
...
for (i in 1:nrow(df_feature)){
  ...
  # extract corresponding PSM ids of for site i
  ...
  # check the number of unique modification counts across all PSMs pertaining to that site
  ...
  for (j in unique_mod_number_i){
    ...
    # generate new feature table entry
